-- LocalScript - Auto Collect Loot v15
...
local function teleportToEscape()
    ...
    local bouncer = exitGateway:FindFirstChild("Bouncer")
    local doorway = exitGateway:FindFirstChild("Doorway")
    ...
    -- Pegar altura do chão do Frame
    local floorY = bouncerPos.Y - (bouncer.Size.Y / 2) + 3
    
    if doorway then
        local frame = doorway:FindFirstChild("Frame")
        if frame then
            -- Procurar Part no Frame para pegar altura do chão
            for _, part in ipairs(frame:GetChildren()) do
                if part:IsA("BasePart") then
                    -- Usar topo da Part como chão + altura do personagem
                    floorY = part.Position.Y + (part.Size.Y / 2) + 3
                    break
                end
            end
        end
    ...
    if doorway then
        local doorwayPos = doorway:GetPivot().Position
        local direction = (doorwayPos - bouncerPos).Unit
        -- -1.5 para afastar do Doorway (mais para dentro)
        offsetX = direction.X * -1.5
        offsetZ = direction.Z * -1.5
    end
    
    root.CFrame = CFrame.new(bouncerPos.X + offsetX, floorY, bouncerPos.Z + offsetZ)
    return true
end
