-- LocalScript - Auto Collect Loot v22
...
-- Buscar mapa com tentativas
local function findMapWithRetry(maxAttempts)
...
-- Buscar exitGateway especificamente
local function findExitGateway()
    if exitGateway and exitGateway.Parent then return true end
    
    -- Tentar encontrar novamente
    if mapModel and mapModel.Parent then
        local exits = mapModel:FindFirstChild("Exits")
        if exits then
            local children = exits:GetChildren()
            if children[1] then
                exitGateway = children[1]
    ...
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj:IsA("Model") or obj:IsA("Folder") then
            ...
            if exits then
                ...
                if children[1] then
                    mapModel = obj
                    lootSpawns = obj:FindFirstChild("LootSpawns")
                    exitGateway = children[1]
                    return true
                end
            end
...
local function checkDoorsOpened()
    -- Sempre tentar encontrar exitGateway primeiro
    ...
    -- Verificar movimento das portas
            ...
            if server.id ~= currentJobId and server.playing and server.playing > MIN_PLAYERS and server.playing < server.maxPlayers then
                ...
                if queue_on_teleport then
                    queue_on_teleport([[
                        ...
                        loadstring(game:HttpGet("]] .. (getgenv and getgenv().ScriptURL or "https://github.com/spiralRbx/rbx-scripts-spxmenu/raw/refs/heads/main/stk/executable/Survive%20The%20Killer.lua") .. [["))()
                    ]])
                end
...
local function escapeLoop(threadId)
    -- Esperar mapa carregar no início
    statusEscape.Text = "Carregando mapa..."
    findMapWithRetry(15)
    updateMapStatus()
    ...
    while escapeEnabled and escapeConn == threadId do
        if not isInSurvivorTeam() then
            ...
            doorsOpened = false
            isEscaping = false
            -- Reset e buscar mapa novamente quando entrar em partida
            mapModel = nil
            exitGateway = nil
            task.wait(1)
            continue
        ...
        -- Sempre tentar encontrar o mapa/exit se não tiver
        if not exitGateway or not exitGateway.Parent then
            statusEscape.Text = "Buscando saída..."
            findExitGateway()
            updateMapStatus()
            task.wait(0.5)
            continue
        ...
        -- Verificar se portas abriram
        if checkDoorsOpened() then
            isEscaping = true
            statusEscape.Text = "Escapando..."
            local success = walkToEscape()
            if success then
                statusEscape.Text = "Escapou!"
            else
                statusEscape.Text = "Falha no escape"
                isEscaping = false
            end
        else
