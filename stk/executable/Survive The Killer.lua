...
Converted["_VectorFrame"].Position = UDim2.new(0.433432311, 34, 0.24257426, 3)
...
Converted["_Inf_map"].TextSize = 24
...
Converted["_Inf_players"].TextSize = 24
...
Converted["_GameTitler"].TextSize = 15
...
local function TGXYF_fake_script() -- Fake Script: StarterGui.VectorGui.VectorFrame.VectorTitler.LocalScript
...
local function ETWIRM_fake_script() -- Fake Script: StarterGui.VectorGui.VectorFrame.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
    script.Parent = Converted["_VectorFrame"]
    local req = require
    local require = function(obj)
        local fake = fake_module_scripts[obj]
        if fake then
...
local function DPFGJ_fake_script() -- Fake Script: StarterGui.VectorGui.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
				...
				if server.id ~= currentJobId and server.playing and server.playing > MIN_PLAYERS and server.playing < server.maxPlayers then
					if queue_on_teleport then
						queue_on_teleport([[
	                        ...
	                        loadstring(game:HttpGet("]] .. (getgenv and getgenv().ScriptURL or "SCRIPT_URL_AQUI") .. [["))()
	                    ]])
					end
					task.wait(1)
...
coroutine.wrap(TGXYF_fake_script)()
coroutine.wrap(ETWIRM_fake_script)()
coroutine.wrap(DPFGJ_fake_script)()