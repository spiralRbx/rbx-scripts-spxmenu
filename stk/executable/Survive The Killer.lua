-- LocalScript - Auto Collect Loot v13
...
local function teleportToEscape()
    ...
    -- Calcular direção para o Doorway
    local offsetX, offsetZ = 0, 0
    
    if doorway then
        local doorwayPos = doorway:GetPivot().Position
        local direction = (doorwayPos - bouncerPos).Unit
        -- Mover 3 studs na direção do Doorway (borda do Bouncer)
        offsetX = direction.X * 3
        offsetZ = direction.Z * 3
    end
