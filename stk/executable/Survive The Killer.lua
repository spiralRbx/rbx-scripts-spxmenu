-- LocalScript - Auto Collect Loot v19
    ...
    if success and servers and servers.data then
        for _, server in ipairs(servers.data) do
            if server.id ~= currentJobId and server.playing and server.playing > MIN_PLAYERS and server.playing < server.maxPlayers then
                ...
                -- Queue script para executar após teleporte
                if queue_on_teleport then
                    queue_on_teleport([[
                        task.wait(5)
                        loadstring(game:HttpGet("https://raw.githubusercontent.com/spiralRbx/rbx-scripts-spxmenu/refs/heads/main/stk/executable/Survive%20The%20Killer.lua"))()
                    ]])
                end
