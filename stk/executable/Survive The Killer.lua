-- LocalScript - Auto Collect Loot v11
...
local door1StartPos, door2StartPos = nil, nil
local doorsOpened = false
local isEscaping = false -- Flag para bloquear outras funções durante escape
...
local function findMap()
    ...
    door1StartPos, door2StartPos = nil, nil
    doorsOpened = false
    isEscaping = false
        ...
        if obj:IsA("Model") or obj:IsA("Folder") then
            ...
            if loots or exits then
                ...
                if exitGateway then
                    ...
                    if doorway then
                        local d1 = doorway:FindFirstChild("Door1")
                        local d2 = doorway:FindFirstChild("Door2")
                        if d1 and d2 then
                            door1StartPos = d1.Position
                            door2StartPos = d2.Position
                        end
                    end
                end
                return true
...
local function checkDoorsOpened()
    ...
    if not door1 or not door2 then return false end
    
    if not door1StartPos or not door2StartPos then
        door1StartPos = door1.Position
        door2StartPos = door2.Position
        return false
    end
    
    local moved1 = (door1.Position - door1StartPos).Magnitude > 0.5
    local moved2 = (door2.Position - door2StartPos).Magnitude > 0.5
    
    if moved1 or moved2 then
        doorsOpened = true
        return true
    end
    return false
...
local function getTrigger()
    ...
    return exitGateway:FindFirstChild("Trigger")
end
    ...
    while escapeEnabled do
        ...
        if exitGateway then
            if checkDoorsOpened() then
                isEscaping = true -- Bloqueia outras funções
                ...
                if trigger then
                    ...
                    statusEscape.Text = "Trigger não encontrado"
                end
            else
...
local function safeLoop()
    while safeEnabled do
        ...
        -- PRIORIDADE: Escape > Evade (não evade durante escape)
        if isEscaping then
