-- LocalScript - Auto Collect Loot v20
...
-- Detectar se veio de server hop
local AUTO_START = getgenv and getgenv().AutoFarmStart or false
...
local function updatePlayerCount()
    local countLabel = mainFrame:FindFirstChild("PlayerCount")
    if countLabel then
...
local function walkToEscape()
    ...
    local walked = 0
    while walked < distance and isEscaping and isInSurvivorTeam() do
...
local function serverHop()
    ...
    if success and servers and servers.data then
        for _, server in ipairs(servers.data) do
            if server.id ~= currentJobId and server.playing and server.playing > MIN_PLAYERS and server.playing < server.maxPlayers then
                ...
                -- Queue script para executar após teleporte com Auto Farm ativo
                if queue_on_teleport then
                    queue_on_teleport([[
                        task.wait(3)
                        getgenv().AutoFarmStart = true
                        loadstring(game:HttpGet("]] .. (getgenv().ScriptURL or "https://raw.githubusercontent.com/spiralRbx/rbx-scripts-spxmenu/refs/heads/main/stk/executable/Survive%20The%20Killer.lua") .. [["))()
                    ]])
                end
    ...
    if queue_on_teleport then
        queue_on_teleport([[
            task.wait(3)
            getgenv().AutoFarmStart = true
            loadstring(game:HttpGet("]] .. (getgenv().ScriptURL or "SCRIPT_URL_AQUI") .. [["))()
        ]])
    end
...
local function startAntiAFK()
    ...
    -- Método 3: Mover câmera levemente
    task.spawn(function()
        while true do
            task.wait(30)
            pcall(function()
                ...
                if camera then
                    ...
                    camera.CFrame = cf * CFrame.Angles(0, math.rad(0.1), 0)
                    task.wait(0.1)
                    camera.CFrame = cf
                end
            end)
...
local function farmLoop()
    while farmEnabled do
        ...
        if playerCount <= MIN_PLAYERS then
            ...
            if farmEnabled and #Players:GetPlayers() <= MIN_PLAYERS then
                serverHop()
            end
        else
...
-- Toggle functions declaradas antes para uso no auto start
...
-- Auto start se veio de server hop
if AUTO_START then
    task.wait(2)
    toggleFarmScript()
    if getgenv then
